		...
		var thermostatDial = (function() {
			...
			return function(targetElement, options) {
				...
				var properties = {
					...
					hvac_states: ['off', 'heating', 'cooling', 'auto', 'eco'],
				 ...
				 */
				var state = {
					target_temperature: ${getTargetTemp()},
					ambient_temperature: ${getTemp()},
					hvac_state: '${getHvacState()}',
					has_leaf: ${state?.hasLeaf},
					away: ${(!state.isPresent)},
					eco: false
				};
...
def getDeviceTile(devNum) {
	try {
		...
		def html = """
			...
			<div id='container'>
				<div id='thermostat${devNum}'></div>
			</div>
			${deviceTstatJs(devNum)}
